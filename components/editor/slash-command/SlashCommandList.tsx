...
import { SLASH_COMMAND_ICON_MAP } from './icons'
    ...
    return (
      <AnimatePresence>
        ...
        >
          ...
          >
            <CommandList ref={listRef} className="max-h-[380px] overflow-y-auto p-1.5">
              ...
              {Object.entries(groups).map(([category, groupItems], groupIdx) => (
                ...
                >
                  {groupItems.map((item) => {
                    const Icon = SLASH_COMMAND_ICON_MAP[item.icon]
                    return (
                      <CommandItem
                        key={item.title}
                        value={item.title}
